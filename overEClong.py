...
def read(code):
    df  = pd.read_csv(code)
    ra  = df['Ch9'].values #right ascension
    df  = pd.read_csv(code)
    dec = df['Ch10'].values #declination
    return ra, dec
...
def plot(RA, Dec, angle):
    ...
    map = ax.pcolormesh(Ra, Dec, z.T, cmap=cm.jet)

    tick_labels = np.array([330, 300, 270, 240, 210, 180, 150, 120, 90, 60, 30])
    ax.set_xticklabels(tick_labels, color='#ffffff')
    ax.set_title('RA-DEC(EC) \n oversampling' + str(angle) + 'degrees \n')
